    ...
    if (searchTerm) {
      ...
      result = result.filter(item => {
        const searchableContent = normalize(`
          ...
          ${item.payment_observation_detail || ''}
        `);
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          ...
          {row.original.is_payment_observed && (
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger>
                ...
                <TooltipContent className="bg-amber-50 border-amber-200 text-amber-900 p-3 rounded-xl shadow-xl max-w-xs">
                  <p className="font-black text-[10px] uppercase tracking-widest mb-1">Pago Observado</p>
                  <p className="text-xs font-medium">{row.original.payment_observation_detail || 'Sin detalle de observación'}</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          )}
              ...
              {filteredData.length > 0 ? (
                filteredData.map((income) => (
                  <Card key={income.id} className={cn(
                    ...
                    income.is_payment_observed && "ring-2 ring-amber-400 ring-inset"
                  )}>
                    <div className={cn(
                      "p-4 border-b border-slate-100 flex justify-between items-center",
                      income.is_payment_observed ? "bg-amber-50" : "bg-slate-50/50"
                    )}>
                      <div className="flex items-center gap-2">
                    ...
                    <CardContent className="p-4 space-y-4">
                      ...
                      {income.is_payment_observed && (
                        <div className="p-3 bg-amber-50 rounded-xl border border-amber-100">
                          ...
                          <p className="text-xs font-medium text-amber-900">{income.payment_observation_detail}</p>
                        </div>
                      )}
