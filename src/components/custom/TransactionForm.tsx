...
export default function TransactionForm({ initialData, onClose, onSuccess }: TransactionFormProps) {
  ...
  const form = useForm<TransactionFormValues>({
    ...
    defaultValues: {
      ...
      is_payment_observed: initialData?.is_payment_observed || false,
      payment_observation_detail: initialData?.payment_observation_detail || '',
    },
  });
    ...
    const fetchSocioName = async () => {
      if (debouncedDni && debouncedDni.length === 8) {
        ...
        try {
          ...
          console.error('Error fetching socio by DNI:', error.message);
          form.setValue('full_name', '', { shouldValidate: true });
          setDniNotFound(true);
          toast.error('Error al buscar socio: ' + error.message);
        } finally {
          setIsSearchingDni(false);
        }
      } else if (debouncedDni.length === 0) {
        form.setValue('full_name', '', { shouldValidate: true });
        setDniNotFound(false);
      }
    };

    // Only fetch if DNI is 8 digits and not an initial load for an existing income
  ...
  const onSubmit = async (values: TransactionFormValues) => {
    ...
    try {
      const dataToSave = {
        ...values,
        payment_observation_detail: values.is_payment_observed ? values.payment_observation_detail : null
      };
              ...
              <FormItem>
                ...
                <FormControl>
                  <Input 
                    ...
                    readOnly={isSearchingDni || (dniValue.length === 8 && !dniNotFound)} // Make read-only if DNI found or searching
                  />
                </FormControl>
                <FormMessage />
              </FormItem>
